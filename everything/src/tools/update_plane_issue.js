    ...
    try {
        ...
        if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(issue_id)) {
            ...
            try {
                ...
                const errorText = JSON.stringify({ error: `Error: Could not resolve issue code ${issue_id} to a UUID. Details: ${e.message}` });
                return { content: [{ type: 'text', text: errorText }], isError: true };
            }
        }
...
export const update_plane_issue = {
